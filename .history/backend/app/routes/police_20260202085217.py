...
from app.schemas.police import PoliceCreate, PoliceUpdate, PoliceResponse
...
def create_police(
    ...
    existing_police = (
        db.query(Police).filter(Police.num_police == data.num_police).first()
    )
    if existing_police:
        raise HTTPException(status_code=400, detail="Ce numéro de police existe déjà.")
...
def delete_police(
    police_id: int, db: Session = Depends(get_db), admin: User = Depends(admin_only)
):
