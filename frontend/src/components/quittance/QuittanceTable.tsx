...
export default function QuittanceTable() {
  ...
  const handleCreateQuittance = async (formData: QuittanceFormData) => {
    try {
      ...
      alert("Les quittances sont créées automatiquement lors de la création d'un devis. \n\nCréez d'abord un devis pour générer une quittance.");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {quittances.length} quittance{quittances.length > 1 ? "s" : ""} · Gestion des paiements
          </p>
        </div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              {sortedQuittances.map((quittance) => (
                <TableRow key={quittance.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {quittance.created_at && (
                          <>Créé le {new Date(quittance.created_at).toLocaleDateString('fr-FR')}</>
                        )}
                      </div>
                    </div>
                  </TableCell>
                    ...
                    <div className="space-y-1.5">
                      <div>
                        <div className="font-medium text-gray-900 dark:text-white text-sm">
                          {quittance.client?.nom_complet || `Client ${formatDevisId(quittance.fk_client_id)}`}
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {quittance.client?.cin && `CIN: ${quittance.client.cin}`}
                        </div>
                      </div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <TableCell className="py-4 px-5">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`font-medium ${quittance.statut_paiement === "annulé" || quittance.statut_paiement === "remboursé"
                      ...
                      {quittance.prime_total > 0 && (
                        <div className="space-y-1">
                          <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <span>{Math.round((quittance.montant_encaisse / quittance.prime_total) * 100)}%</span>
                          </div>
                          <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-green-500 rounded-full transition-all duration-300"
                              style={{ width: `${(quittance.montant_encaisse / quittance.prime_total) * 100}%` }}
                            />
                          </div>
                        </div>
                      )}
                  ...
                  <TableCell className="py-4 px-5">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button
                        ...
                        disabled={quittance.statut_paiement === "annulé" || quittance.statut_paiement === "remboursé"}
                        className={`p-2 ${quittance.statut_paiement === "annulé" || quittance.statut_paiement === "remboursé"
                        ...
                        title={
                          quittance.statut_paiement === "annulé" || quittance.statut_paiement === "remboursé"
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 px-5 py-3">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                Affichage de <span className="font-medium">{quittances.length}</span> quittance{quittances.length > 1 ? 's' : ''}
              </div>
              <div className="flex items-center gap-2">
