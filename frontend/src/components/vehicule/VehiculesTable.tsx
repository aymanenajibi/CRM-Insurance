      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {vehicules.length} véhicule{vehicules.length > 1 ? "s" : ""} · Gestion du parc automobile
          </p>
        </div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              {sortedVehicules.map((vehicule) => {
                ...
                const dateMiseEnCirculation = new Date(vehicule.date_mise_en_circulation);
                const maintenant = new Date();
                const ageEnAnnees = maintenant.getFullYear() - dateMiseEnCirculation.getFullYear();

                return (
                  <TableRow key={vehicule.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <TableCell className="py-4 px-5">
                      <div className="space-y-1.5">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {ageEnAnnees} an{ageEnAnnees > 1 ? 's' : ''} d'ancienneté
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="py-4 px-5">
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 px-5 py-3">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                Affichage de <span className="font-medium">{vehicules.length}</span> véhicule{vehicules.length > 1 ? 's' : ''}
              </div>
              <div className="flex items-center gap-2">
